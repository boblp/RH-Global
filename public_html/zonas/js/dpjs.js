...
if(datePicker.languageinfo != 'en') {
        ...
        for(var i = 0, scriptFile; scriptFile = scriptFiles[i]; i++) {
                if(scriptFile.src && scriptFile.src.match(/dpjs/)) {
                        loc = scriptFile.src.replace("dpjs", "lang/" + datePicker.languageinfo);
                        break;
                }
        }
...
function datePicker(options) {
        ...
        o.fade = function() {
                ...
                var diff = Math.round(o.opacity + ((o.opacityTo - o.opacity)/2));

                o.setOpacity(diff);

                if(Math.abs(o.opacityTo - diff) > 3) {
                        o.fadeTimer = window.setTimeout(function () { o.fade(); }, 10);
                } else {
...
// Aqui le cambié los botones para que solo sean meses
/*                // previous year
...
// HASTA Aqui le cambié los botones para que solo sean meses (Para regresarlo, copiar y pegar toda la sección desde el original)
...
function eventsetup() {
  var rd1 = document.getElementById("scope1");
  var rd2 = document.getElementById("scope2");
  var rf1 = document.getElementById("finicio");
  var rf2 = document.getElementById("ffin");
  var rs1 = document.getElementById("fsel1");
  var rs2 = document.getElementById("fsel2");
  
  rf1.setAttribute("autocomplete","off");
  rf2.setAttribute("autocomplete","off");
  rd1.onclick = function() {
    muestrasel("sel1");
    if(rs1.selectedIndex==0) {
      mwHide("id:botonok");
    } else {
      mwShow("id:botonok","");
    }
  ...
  rd2.onclick = function() {
    muestrasel("sel2");
    if(rs2.selectedIndex==0) {
      mwHide("id:botonok");
    } else {
      mwShow("id:botonok","");
    }
  }
  rf1.onfocus = function() {
    if(this.value>rf2.value) {
      rf2.value = this.value;
    }
    this.blur();
  }
  rf2.onfocus = function() {
    if(this.value<rf1.value) {
      rf1.value = this.value;
    }
    this.blur();
    return false;
  }
  
  rs1.onchange = function() {
  ...
  if(rd1.checked && rs1.selectedIndex>0) {
    mwShow("id:botonok","");
    muestrasel("sel1");
  }
  if(rd2.checked && rs2.selectedIndex>0) {
    mwShow("id:botonok","");
    muestrasel("sel2");
  }
}

function muestrasel(cual) {
  mwShow2("id:" + cual,"class:togglesel");
}

function muestraforma() {
  mwShow("class:ocultaform","id:toggleform");
}

addLoad(eventsetup);

function getfolio(superid) {
  var cf = document.getElementById("idfolio");
  if(cf.value * 1 == cf.value && cf.value>0) {
    var candid = cf.value * 1;
    var verifid = (superid*candid) + 1024;
    window.open("/repview/" + superid + "/" + candid + "/" + verifid, "", "width=300,height=250,scrollbars=no,menubar=no,menu=no,status=no,statusbar=no");
  } else {
    alert(cf.value * 1);
  }
}

function getedc(tid,ver) {
  var zoneSelect = document.getElementById("fsel1");
  var curZone = zoneSelect.options[zoneSelect.selectedIndex].value;
  if(curZone!="0") {
    tid = curZone;
  }
  var targeturl = "http://servicios.rhglobal.com.mx";
  var fi = document.getElementById("finicio");
  var ff = document.getElementById("ffin");
  var fiv = fi.value.replace(/-/g,"");
  var ffv = ff.value.replace(/-/g,"");
  window.open(targeturl + "/edc/s/" + tid + "/" + ver + "/1O" + fiv + "O" + ffv + ".pdf", "", "width=50,height=50,scrollbars=no,menubar=no,menu=no,status=no,statusbar=no");
}
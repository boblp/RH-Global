...
// con licencia LGPL
	...
	function randchain($min,$max=false) {
		$max = ($max) ? $max : $min;
		$chain = "";
		while(strlen($chain)<$min) {
			$chain.= preg_replace("/[^a-z]/","",strtolower(base64_encode(md5(microtime()."lmgs"))));
		}
		if(strlen($chain)>$max) {
			$chain = substr($chain,0,$max);
		}
		return $chain;
	}

	function checkdates($ini,$fin,$maxdays=false) {
		$dini = array("y"=>substr($ini,0,4),"m"=>substr($ini,4,2),"d"=>substr($ini,6,2));
		$dfin = array("y"=>substr($fin,0,4),"m"=>substr($fin,4,2),"d"=>substr($fin,6,2));
		if(intval($dini["y"])<2005 || intval($dini["y"])>date("Y") || !checkdate($dini["m"],$dini["d"],$dini["y"]) || intval($dfin["y"])<2005 || intval($dfin["y"])>date("Y") || !checkdate($dfin["m"],$dfin["d"],$dfin["y"])) {
			return false;
		}
		$tini = mktime(0,0,0,$dini["m"],$dini["d"],$dini["y"]);
		$tfin = mktime(23,59,59,$dfin["m"],$dfin["d"],$dfin["y"]);
		if($tfin<$tini) {
			$tini = mktime(0,0,0,$dfin["m"],$dfin["d"],$dfin["y"]);
			$tfin = mktime(23,59,59,$dini["m"],$dini["d"],$dini["y"]);
		}
		if($maxdays && (($tfin-$tini)/86400)>$maxdays) {
			return false;
		}
		return array("i"=>date("Y-m-d H:i:s",$tini),"f"=>date("Y-m-d H:i:s",$tfin));
	}
